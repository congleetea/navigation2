  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  if (use_rotate_to_heading_ && allow_reversing_) {
    RCLCPP_WARN(
      ...
      "parameter cannot be set to true. By default setting "
...
double RegulatedPurePursuitController::costAtPose(const double & x, const double & y)
{
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    RCLCPP_FATAL(
      logger_,
      "The dimensions of the costmap is too small to fully "
      "include your robot's footprint, "
...
{
  // Iterating through the transformed global path to determine the position of
  // the cusp
...
PLUGINLIB_EXPORT_CLASS(
  nav2_regulated_pure_pursuit_controller::RegulatedPurePursuitController, nav2_core::Controller)
