...
protected:
  ...
  void SetUp()
  {
    ...
    path_poses_pub_orig_ =
      node_lifecycle_->create_publisher<geometry_msgs::msg::PoseArray>("/plan_poses_original", 100);
    costmap_pub_ = std::make_shared<nav2_costmap_2d::Costmap2DPublisher>(
      node_lifecycle_, costmap_sub_->getCostmap().get(), "map", "/costmap", true);
    ...
    node_lifecycle_->set_parameter(
      rclcpp::Parameter("SmoothPath.cost_check_points", std::vector<double>()));
    reloadParams();
  }
  ...
  typedef std::function<double(
    int i, const Eigen::Vector3d & prev_p, const Eigen::Vector3d & p,
   ...
   * @param criterion Criterion of path quality. Total path quality =
   * sqrt(sum(criterion(data[i])^2)/count(data))
   * @return Percentage of improvement (relative to input path quality)
   */
  double assessPathImprovement(
    const std::vector<Eigen::Vector3d> & input, const std::vector<Eigen::Vector3d> & output,
    const QualityCriterion3 & criterion, const QualityCriterion3 * criterion_out = nullptr)
  {
    if (!criterion_out) {
  ...
  double assessPathImprovement(
    const std::vector<Eigen::Vector3d> & input, const std::vector<Eigen::Vector3d> & output,
    const QualityCriterion2 & criterion, const QualityCriterion2 * criterion_out = nullptr)
  {
    if (!criterion_out) {
      criterion_out = &criterion;
    }
  ...
  double assessPathImprovement(
    const std::vector<Eigen::Vector3d> & input, const std::vector<Eigen::Vector3d> & output,
    const QualityCriterion1 & criterion, const QualityCriterion1 * criterion_out = nullptr)
  {
    if (!criterion_out) {
      criterion_out = &criterion;
    }
  ...
  std::vector<Eigen::Vector3d> zigZaggedPath(
    const std::vector<Eigen::Vector3d> & input, double offset)
  {
    auto output = input;
    for (size_t i = 1; i < input.size() - 1; i++) {
      ...
      output[i].block<2, 1>(0, 0) +=
        Eigen::Vector2d(direction[1], -direction[0]) * offset * (i % 2 == 0 ? 1.0 : -1.0);
    }
    return output;
  }
  ...
  int cusp_i_ = -1;
  QualityCriterion3 mvmt_smoothness_criterion_ = [this](
                                                   int i, const Eigen::Vector3d & prev_p,
...
TEST_F(SmootherTest, testingSmoothness)
{
  ...
  std::vector<Eigen::Vector3d> sharp_turn_90 = {
    ...
    {0.3, 0, M_PI / 4},  {0.3, 0.1, M_PI / 2}, {0.3, 0.2, M_PI / 2},
    {0.3, 0.3, M_PI / 2}};
  ...
  orientation_smoothness_improvement = assessPathImprovement(
    sharp_turn_90_then_reverse, smoothed_path, orientation_smoothness_criterion);
  EXPECT_GT(orientation_smoothness_improvement, 0.0);
...
TEST_F(SmootherTest, testingAnchoringToOriginalPath)
{
  ...
  auto origin_similarity_criterion = [&sharp_turn_90](int i, const Eigen::Vector3d & p) {
...
TEST_F(SmootherTest, testingObstacleAvoidance)
{
  ...
  auto cost_avoidance_criterion = [&collision_checker, &footprint](int, const Eigen::Vector3d & p) {
  ...
  std::vector<Eigen::Vector3d> straight_near_obstacle = {
    {0.05, 0.05, 0}, {0.45, 0.05, 0}, {0.85, 0.05, 0}, {1.25, 0.05, 0},
    {1.65, 0.05, 0}, {2.05, 0.05, 0}, {2.45, 0.05, 0}, {2.85, 0.05, 0},
    {3.25, 0.05, 0}, {3.65, 0.05, 0}, {4.05, 0.05, 0}};

  std::vector<Eigen::Vector3d> smoothed_path;
  EXPECT_TRUE(smoothPath(straight_near_obstacle, smoothed_path));

  // we don't expect result to be smoother than original as original straight
  // line was 100% smooth but let's check for large discontinuities using a well
  // chosen upper bound
  auto upper_bound = zigZaggedPath(straight_near_obstacle, 0.01);
  EXPECT_GT(assessPathImprovement(upper_bound, smoothed_path, mvmt_smoothness_criterion_), 0.0);

  double cost_avoidance_improvement =
    assessPathImprovement(straight_near_obstacle, smoothed_path, cost_avoidance_criterion);
...
TEST_F(SmootherTest, testingObstacleAvoidanceNearCusps)
{
  ...
  // path feasibility can be restored by subsequent resmoothing with higher
  // w_curve
  ...
  EXPECT_GT(assessPathImprovement(upper_bound, smoothed_path, mvmt_smoothness_criterion_), 0.0);
  double cost_avoidance_improvement_simple =
    assessPathImprovement(cusp_near_obstacle, smoothed_path, cost_avoidance_criterion);
  EXPECT_GT(cost_avoidance_improvement_simple, 0.0);
  EXPECT_NEAR(cost_avoidance_improvement_simple, 42.6, 1.0);
  double worst_cost_improvement_simple =
    assessWorstPoseImprovement(cusp_near_obstacle, smoothed_path, cost_avoidance_criterion);
  RCLCPP_INFO(
  ...
  double cost_avoidance_improvement_extra_careful_cusp =
    assessPathImprovement(cusp_near_obstacle, smoothed_path_ecc, cost_avoidance_criterion);
  ...
  double worst_cost_improvement_extra_careful_cusp =
    assessWorstPoseImprovement(cusp_near_obstacle, smoothed_path_ecc, cost_avoidance_criterion);
  RCLCPP_INFO(
  ...
  // although extra careful cusp optimization avoids cost better than simple
  // one, overall the path doesn't need to deflect so much from original, since
  // w_cost is smaller and thus the obstacles are avoided mostly in dangerous
  // zones around cusps
  auto origin_similarity_criterion = [&cusp_near_obstacle](int i, const Eigen::Vector3d & p) {
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("ceres_smoother"),
    "Original similarity improvement (cusp, ecc vs. simple): %lf", origin_similarity_improvement);
  EXPECT_GT(origin_similarity_improvement, 0.0);
  ...
  // reset parameters back to homogeneous and shift cost check point to the
  // center of the footprint
  node_lifecycle_->set_parameter(rclcpp::Parameter("SmoothPath.w_smooth", 15000.0));
  node_lifecycle_->set_parameter(rclcpp::Parameter("SmoothPath.w_curve", 1.0));
  node_lifecycle_->set_parameter(rclcpp::Parameter("SmoothPath.w_cost", 0.015));
  node_lifecycle_->set_parameter(rclcpp::Parameter("SmoothPath.cusp_zone_length", -1.0));
  node_lifecycle_->set_parameter(rclcpp::Parameter(
  ...
  cost_avoidance_improvement_simple =
    assessPathImprovement(cusp_near_obstacle, smoothed_path, cost_avoidance_criterion);
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("ceres_smoother"),
    "Cost avoidance improvement (cusp_shifted, simple): %lf, %lf",
    cost_avoidance_improvement_simple, worst_cost_improvement_simple);
  ...
  double cost_avoidance_improvement_shifted_cost_check =
    assessPathImprovement(cusp_near_obstacle, smoothed_path_scc, cost_avoidance_criterion);
  ...
  double worst_cost_improvement_shifted_cost_check =
    assessWorstPoseImprovement(cusp_near_obstacle, smoothed_path_scc, cost_avoidance_criterion);
  RCLCPP_INFO(
    rclcpp::get_logger("ceres_smoother"),
    "Cost avoidance improvement (cusp_shifted, scc): %lf, %lf",
  ...
  EXPECT_GT(
    assessPathImprovement(upper_bound, smoothed_path_scce, mvmt_smoothness_criterion_), 0.0);
  double cost_avoidance_improvement_shifted_extra =
    assessPathImprovement(cusp_near_obstacle, smoothed_path_scce, cost_avoidance_criterion);
  double worst_cost_improvement_shifted_extra =
    assessWorstPoseImprovement(cusp_near_obstacle, smoothed_path_scce, cost_avoidance_criterion);
  RCLCPP_INFO(
    rclcpp::get_logger("ceres_smoother"),
    "Cost avoidance improvement (cusp_shifted, scce): %lf, %lf",
  ...
  cost_avoidance_improvement_extra_careful_cusp =
    assessPathImprovement(cusp_near_obstacle, smoothed_path_ecc, cost_avoidance_criterion);
  worst_cost_improvement_extra_careful_cusp =
    assessWorstPoseImprovement(cusp_near_obstacle, smoothed_path_ecc, cost_avoidance_criterion);
  EXPECT_GT(cost_avoidance_improvement_extra_careful_cusp, 0.0);
  RCLCPP_INFO(
    rclcpp::get_logger("ceres_smoother"),
    "Cost avoidance improvement (cusp_shifted, ecc): %lf, %lf",
    cost_avoidance_improvement_extra_careful_cusp, worst_cost_improvement_extra_careful_cusp);
  EXPECT_NEAR(cost_avoidance_improvement_extra_careful_cusp, 48.5, 1.0);
  EXPECT_GT(
    cost_avoidance_improvement_shifted_extra, cost_avoidance_improvement_extra_careful_cusp);
...
TEST_F(SmootherTest, testingDownsamplingUpsampling)
{
  ...
  QualityCriterion3 mvmt_smoothness_criterion_out = [&cusp_i_out](
                                                      int i, const Eigen::Vector3d & prev_p,
                                                      const Eigen::Vector3d & p,
                                                      const Eigen::Vector3d & next_p) {
    Eigen::Vector2d prev_mvmt = p.block<2, 1>(0, 0) - prev_p.block<2, 1>(0, 0);
  ...
  double smoothness_improvement = assessPathImprovement(
    smoothed_path_downsampled, smoothed_path, mvmt_smoothness_criterion_,
...
TEST_F(SmootherTest, testingCostCheckPointsParamValidity)
{
  ...
  node_lifecycle_->set_parameter(
    rclcpp::Parameter("SmoothPath.cost_check_points", std::vector<double>({0, 0})));
...
TEST_F(SmootherTest, testingLinearSolverTypeParamValidity)
{
  ...
  node_lifecycle_->set_parameter(
    rclcpp::Parameter("SmoothPath.optimizer.linear_solver_type", "DENSE_QR"));
  reloadParams();

  node_lifecycle_->set_parameter(
    rclcpp::Parameter("SmoothPath.optimizer.linear_solver_type", "INVALID_SOLVER"));
  EXPECT_THROW(reloadParams(), std::runtime_error);
}
