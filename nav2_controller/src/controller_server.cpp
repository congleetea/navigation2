...
nav2_util::CallbackReturn ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (
    costmap_ros_->get_current_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE) {
...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  auto find_closest_pose_idx = [&pose, &current_path]() {
    ...
    for (size_t curr_idx = 0; curr_idx < current_path.poses.size(); ++curr_idx) {
      double curr_dist =
        nav2_util::geometry_utils::euclidean_distance(pose, current_path.poses[curr_idx]);
...
bool ControllerServer::isGoalReached()
{
  ...
  nav_2d_utils::transformPose(
    costmap_ros_->getTfBuffer(), costmap_ros_->getGlobalFrameID(), end_pose_, transformed_end_pose,
    tolerance);
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    // If we are trying to change the parameter of a plugin we can just skip it
    // at this point as they handle parameter changes themselves and don't need
    // to lock the mutex
    ...
    if (!dynamic_params_lock_.try_lock()) {
      RCLCPP_WARN(
        get_logger(),
        "Unable to dynamically change Parameters while "
        "the controller is currently running");
      result.successful = false;
      result.reason =
        "Unable to dynamically change Parameters while the "
