...
public:
  ...
  explicit ServiceClient(
    const std::string & service_name, const rclcpp::Node::SharedPtr & provided_node)
    ...
    const std::chrono::nanoseconds timeout = std::chrono::nanoseconds(-1))
  {
    ...
    if (
      callback_group_executor_.spin_until_future_complete(future_result, timeout) !=
   ...
   */
  bool invoke(
    typename RequestType::SharedPtr & request, typename ResponseType::SharedPtr & response)
  {
    while (!client_->wait_for_service(std::chrono::seconds(1))) {
      if (!rclcpp::ok()) {
        throw std::runtime_error(
