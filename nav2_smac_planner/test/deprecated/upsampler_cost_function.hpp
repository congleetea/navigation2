 ...
 * unconstrained optimization including curvature, smoothness, collision, and
 * avoid obstacles.
 */
...
public:
  ...
  // TODO(stevemacenski) removed upsample_ratio because temp upsampling on path
  // size
  ...
  virtual bool Evaluate(const double * parameters, double * cost, double * gradient) const
  {
    ...
    for (int i = 0; i != NumParameters() / 2; i++) {
      ...
      xi_p1 = _path.at(i + 1);
      xi_m1 = _path.at(i - 1);
      // xi_p1 = Eigen::Vector2d(parameters[x_index + 2], parameters[y_index +
      // 2]); xi_m1 = Eigen::Vector2d(parameters[x_index - 2],
      // parameters[y_index - 2]);
