...
public:
  ...
  struct AnalyticExpansionNode
  {
    AnalyticExpansionNode(
      NodePtr & node_in, Coordinates & initial_coords_in, Coordinates & proposed_coords_in)
  ...
  /**
   * @brief Sets the collision checker and costmap to use in expansion
   * validation
   ...
   * @param best_cost Best heuristic cost to propertionally expand more closer
   * to the goal
   ...
   * @return A set of analytically expanded nodes to the goal from current node,
   * if possible
   */
  AnalyticExpansionNodes getAnalyticPath(
    const NodePtr & node, const NodePtr & goal, const NodeGetter & getter);

  /**
   * @brief Takes final analytic expansion and appends to current expanded node
   * @param node The node to start the analytic path from
   * @param goal The goal node to plan to
   * @param expanded_nodes Expanded nodes to append to end of current search
   * path
   * @return Node pointer to goal node if successful, else return nullptr
   */
  NodePtr setAnalyticPath(
    const NodePtr & node, const NodePtr & goal, const AnalyticExpansionNodes & expanded_nodes);
