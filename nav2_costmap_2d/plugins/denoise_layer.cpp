  ...
  if (group_connectivity_type_param == 4) {
    ...
    if (group_connectivity_type_param != 8) {
      RCLCPP_WARN(
        ...
        "or 8 (neighbors pixels are connected horizontally, vertically and "
        "diagonally)."
...
void DenoiseLayer::reset() { current_ = false; }

bool DenoiseLayer::isClearable() { return false; }

void DenoiseLayer::updateBounds(
  double /*robot_x*/, double /*robot_y*/, double /*robot_yaw*/, double * /*min_x*/,
  double * /*min_y*/, double * /*max_x*/, double * /*max_y*/)
{
}

void DenoiseLayer::updateCosts(
...
void DenoiseLayer::removeSinglePixels(Image<uint8_t> & image) const
{
  ...
  // check any obstacle nearby. In this case, we interpret NO_INFORMATION as an
  // empty space.
  if (!no_information_is_obstacle_) {
    ...
    auto max = [&](const std::initializer_list<uint8_t> lst) {
      std::array<uint8_t, 3> buf = {
        replace_to_free(*lst.begin()), replace_to_free(*(lst.begin() + 1)),
