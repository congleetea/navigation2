...
protected:
  ...
  inline static bool isBackground(uint8_t pixel) { return pixel == BACKGROUND_CODE; }

  inline Image<uint8_t> makeChessboardLikeImage(
    size_t rows, size_t cols, std::vector<uint8_t> & buffer) const;
...
TEST_F(ConnectedComponentsTester, way4EmptyTest)
{
  Image<uint8_t> empty;
  const auto result =
    connectedComponents<ConnectivityType::Way4>(empty, buffer_, label_trees_, isBackground);
...
TEST_F(ConnectedComponentsTester, way8ImageStepsTest)
{
  ...
  const Image<uint8_t> expected_labels = imageFromString<uint8_t>(
    ...
    ".....b"
    "....b.",
    image_buffer_bytes2_, makeLabelsMap('b'));

  const auto result =
    connectedComponents<ConnectivityType::Way8>(input, buffer_, label_trees_, isBackground);
...
TEST_F(ConnectedComponentsTester, way4ImageSieveTest)
{
  ...
  const Image<uint8_t> expected_labels = imageFromString<uint8_t>(
    ...
    ".i.j."
    "k.l.m",
    image_buffer_bytes2_, makeLabelsMap('m'));
...
bool ConnectedComponentsTester::fingerTest()
{
  ...
  const Image<uint8_t> expected_labels = imageFromString<uint8_t>(
    ...
    "a.b.c"
    "a.b.c",
    image_buffer_bytes2_, makeLabelsMap('c'));
...
uint8_t max_list(std::initializer_list<uint8_t> lst) { return std::max(lst); }
