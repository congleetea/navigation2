...
/**
 * @class DenoiseLayer
 * @brief Layer filters noise-induced standalone obstacles (white costmap
 * pixels) or small obstacles groups
 */
...
public:
  ...
  /**
   * @brief Filters noise-induced obstacles in the selected region of the
   * costmap The method is called when costmap recalculation is required. It
   * updates the costmap within its window bounds.
...
private:
  /**
   * @brief Removes from the image single obstacles (white pixels) or small
   * obstacles groups
   *
   * Pixels less than 255 will be interpreted as background (free space), 255 -
   * as obstacles. Replaces groups of obstacles smaller than minimal_group_size_
   * to free space. Groups connectivity type is determined by the connectivity
   * parameter.
   *
   * If minimal_group_size_ is 1 or 0, it does nothing
   * (all standalone obstacles will be preserved, since it satisfies this
   * condition). If minimal_group_size_ equals 2, performs fast filtering based
   * on the dilation operation. Otherwise, it performs a slower
   * segmentation-based operation.
   * @throw std::logic_error in case inner logic errors
   */
  void denoise(Image<uint8_t> & image) const;

  /**
   * @brief Removes from the image groups of white pixels smaller than
   * minimal_group_size_ Segments the image into groups of connected pixels
   * Replace pixels in groups whose size smaller than minimal_group_size_ to
   * zero value (background)
   * @throw std::logic_error in case inner logic errors
   ...
   * Works similarly to removeGroups with minimal_group_size_ = 2, but about 10x
   * faster
   * @throw std::logic_error in case inner logic errors
   * @warning If image.empty() the behavior is undefined
   */
   ...
   * @return true if the pixel value is not an obstacle code. False in other
   * case
   */
...
private:
  ...
  // Pixels connectivity type. Determines how pixels belonging to the same group
  // can be arranged
