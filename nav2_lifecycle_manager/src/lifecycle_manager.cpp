...
bool LifecycleManager::startup()
{
  ...
  if (
    !changeStateForAllNodes(Transition::TRANSITION_CONFIGURE) ||
...
bool LifecycleManager::reset(bool hard_reset)
{
  ...
  if (
    !changeStateForAllNodes(Transition::TRANSITION_DEACTIVATE, hard_reset) ||
...
void LifecycleManager::createBondTimer()
{
  ...
  bond_timer_ = this->create_wall_timer(
    200ms, std::bind(&LifecycleManager::checkBondConnections, this), callback_group_);
}
...
void LifecycleManager::checkBondConnections()
{
  ...
  for (auto & node_name : node_names_) {
    ...
    if (bond_map_[node_name]->isBroken()) {
      ...
      // if a server crashed, it won't get cleared due to failed transition,
      // clear manually
      bond_map_.clear();
...
void LifecycleManager::checkBondRespawnConnection()
{
  ...
  // condition. If another outside user actives the system again, this should
  // not process.
  ...
  if (live_servers == max_live_servers) {
    message(
      "Successfully re-established connections from server respawns, "
      "starting back up.");
    bond_respawn_start_time_ = rclcpp::Time(0);
    bond_respawn_timer_.reset();
    startup();
    ...
    message(
      "Failed to re-establish connection from a server crash after "
      "maximum timeout.");
    bond_respawn_start_time_ = rclcpp::Time(0);
    bond_respawn_timer_.reset();
  }
