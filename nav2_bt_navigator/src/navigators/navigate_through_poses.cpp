  ...
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_through_poses_bt_xml")) {
    ...
    node->declare_parameter<std::string>(
      "default_nav_through_poses_bt_xml", pkg_share_dir +
                                            "/behavior_trees/"
...
void NavigateThroughPosesNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  if (
    ...
    RCLCPP_WARN(
      logger_,
      "Preemption request was rejected since the requested "
      "BT XML file is not the same "
      "as the one that the current goal is executing. "
      "Preemption with a new BT is invalid "
      "since it would require cancellation of the previous "
      "goal instead of true preemption."
      "\nCancel the current goal and send a new action "
      "request if you want to use a "
      "different BT XML file. For now, continuing to track "
      "the last goal until completion.");
...
void NavigateThroughPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  if (goal->poses.size() > 0) {
    RCLCPP_INFO(
      logger_,
      "Begin navigating from current location through %zu poses to "
      "(%.2f, %.2f)",
