...
InputAtWaypoint::InputAtWaypoint() : input_received_(false), is_enabled_(true), timeout_(10.0, 0.0)
{
}
  ...
  const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, const std::string & plugin_name)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".timeout", rclcpp::ParameterValue(10.0));
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".enabled", rclcpp::ParameterValue(true));
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".input_topic", rclcpp::ParameterValue("input_at_waypoint/input"));
  ...
  RCLCPP_INFO(logger_, "InputAtWaypoint: Subscribing to input topic %s.", input_topic.c_str());
...
PLUGINLIB_EXPORT_CLASS(nav2_waypoint_follower::InputAtWaypoint, nav2_core::WaypointTaskExecutor)
