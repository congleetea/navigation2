  ...
  const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, const std::string & plugin_name)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".image_topic", rclcpp::ParameterValue("/camera/color/image_raw"));
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".save_dir", rclcpp::ParameterValue("/tmp/waypoint_images"));
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name + ".image_format", rclcpp::ParameterValue("png"));
  ...
  // get inputted save directory and make sure it exists, if not log and create
  // it
  ...
  try {
    if (!std::experimental::filesystem::exists(save_dir_)) {
      ...
      if (!std::experimental::filesystem::create_directory(save_dir_)) {
        RCLCPP_ERROR(
          logger_,
          "Failed to create directory!: %s required by photo at "
          "waypoint plugin, "
  ...
  if (!is_enabled_) {
    ...
    RCLCPP_INFO(
      logger_,
      "Initializing photo at waypoint plugin, subscribing to camera "
      "topic named; %s",
      image_topic_.c_str());
...
{
  if (!is_enabled_) {
    RCLCPP_WARN(logger_, "Photo at waypoint plugin is disabled. Not performing anything");
    return true;
  }
  try {
    // construct the full path to image filename
    std::experimental::filesystem::path file_name = std::to_string(curr_waypoint_index) + "_" +
                                                    std::to_string(curr_pose.header.stamp.sec) +
                                                    "." + image_format_;
    ...
    RCLCPP_INFO(logger_, "Photo has been taken sucessfully at waypoint %i", curr_waypoint_index);
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      logger_,
      "Couldn't take photo at waypoint %i! Caught exception: %s \n"
      "Make sure that the image topic named: %s is valid and active!",
      curr_waypoint_index, e.what(), image_topic_.c_str());
    return false;
  }
  return true;
}
...
PLUGINLIB_EXPORT_CLASS(nav2_waypoint_follower::PhotoAtWaypoint, nav2_core::WaypointTaskExecutor)
