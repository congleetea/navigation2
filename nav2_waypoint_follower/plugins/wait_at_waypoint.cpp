...
{
WaitAtWaypoint::WaitAtWaypoint() : waypoint_pause_duration_(0), is_enabled_(true) {}
  ...
  const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, const std::string & plugin_name)
{
  ...
  if (waypoint_pause_duration_ == 0) {
    ...
    RCLCPP_INFO(
      logger_,
      "Waypoint pause duration is set to zero, disabling "
  ...
  const geometry_msgs::msg::PoseStamped & /*curr_pose*/, const int & curr_waypoint_index)
{
  ...
  RCLCPP_INFO(
    logger_, "Arrived at %i'th waypoint, sleeping for %i milliseconds", curr_waypoint_index,
    waypoint_pause_duration_);
  rclcpp::sleep_for(std::chrono::milliseconds(waypoint_pause_duration_));
  return true;
}
}  // namespace nav2_waypoint_follower
PLUGINLIB_EXPORT_CLASS(nav2_waypoint_follower::WaitAtWaypoint, nav2_core::WaypointTaskExecutor)
