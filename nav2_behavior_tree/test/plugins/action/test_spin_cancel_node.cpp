...
{
public:
  CancelSpinServer() : TestActionServer("spin") {}
...
public:
  static void SetUpTestCase()
  {
    ...
    client_ = rclcpp_action::create_client<nav2_msgs::action::Spin>(node_, "spin");

    BT::NodeBuilder builder = [](const std::string & name, const BT::NodeConfiguration & config) {
      return std::make_unique<nav2_behavior_tree::SpinCancel>(name, "spin", config);
    };
...
std::shared_ptr<CancelSpinServer> CancelSpinActionTestFixture::action_server_ = nullptr;
...
std::shared_ptr<BT::BehaviorTreeFactory> CancelSpinActionTestFixture::factory_ = nullptr;
...
int main(int argc, char ** argv)
{
  ...
  std::thread server_thread([]() { rclcpp::spin(CancelSpinActionTestFixture::action_server_); });
