 ...
 *
 * If the current child return failure, the next child is ticked and if the last
 * child returns failure, the first child is ticked and the cycle continues
 ...
 * RUNNING
 *                                  - RoundRobin returns RUNNING and no other
 * nodes are ticked. | FAILURE | RUNNING |  IDLE   |  - A returns FAILURE so B
 * gets ticked and returns RUNNING
 *                                  - RoundRobin returns RUNNING and C is not
 * ticked yet | FAILURE | SUCCESS |  IDLE   |  - B returns SUCCESS, so
 * RoundRobin halts all children and
 *                                  - returns SUCCESS, next iteration will tick
 * C. | RUNNING |  IDLE   | FAILURE |  - C returns FAILURE, so RoundRobin
 * circles and ticks A.
 *                                  - A returns RUNNING, so RoundRobin returns
 * RUNNING.
 *
