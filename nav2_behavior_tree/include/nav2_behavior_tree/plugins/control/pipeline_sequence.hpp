 ...
 *
 * Tick All Previous Again means every node up till this one will be reticked.
 * Even if a previous node returns Running, the next node will be reticked.
 *
 ...
 * --------------------------------
 * |  IDLE   |  IDLE   |  IDLE   |
 * | RUNNING |  IDLE   |  IDLE   |  - at first A gets ticked. Assume it returns
 * RUNNING
 *                                  - PipelineSequence returns RUNNING and no
 * other nodes are ticked. | SUCCESS | RUNNING |  IDLE   |  - This time A
 * returns SUCCESS so B gets ticked as well
 *                                  - PipelineSequence returns RUNNING and C is
 * not ticked yet | RUNNING | SUCCESS | RUNNING |  - A gets ticked and returns
 * RUNNING, but since it had previously
 *                                  - returned SUCCESS, PipelineSequence
 * continues on and ticks B.
 *                                  - Since B also returns SUCCESS, C gets
 * ticked this time as well. | RUNNING | SUCCESS | SUCCESS |  - A is still
 * RUNNING, and B returns SUCCESS again. This time C
 *                                  - returned SUCCESS, ending the sequence.
 * PipelineSequence
 *                                  - returns SUCCESS and halts A.
 *
 * If any children at any time had returned FAILURE. PipelineSequence would have
 * returned FAILURE and halted all children, ending the sequence.
 *
