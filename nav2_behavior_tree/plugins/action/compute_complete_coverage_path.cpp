  ...
  if (getInput("file_field", gml_filename)) {
    ...
    for (unsigned int i = 0; i != polys.size(); i++) {
      for (unsigned int j = 0; j != polys[i].points.size(); j++) {
        nav2_msgs::msg::Coordinate coord;
...
BT::NodeStatus ComputeCoveragePathAction::on_aborted()
{
  ...
  nav2_msgs::msg::PathComponents cov_path;
...
BT_REGISTER_NODES(factory)
{
  BT::NodeBuilder builder =
    ...
    {
      return std::make_unique<nav2_behavior_tree::ComputeCoveragePathAction>(
        name, "compute_coverage_path", config);
    };

  factory.registerBuilder<nav2_behavior_tree::ComputeCoveragePathAction>(
