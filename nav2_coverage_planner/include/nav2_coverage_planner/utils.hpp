...
#include "nav2_coverage_planner/types.hpp"
 ...
 */
inline nav2_msgs::msg::PathComponents toCoveragePathMsg(
  const Swaths & raw_swaths, const F2CField & field, const bool ordered,
  const std_msgs::msg::Header & header, const bool is_cartesian)
{
  nav2_msgs::msg::PathComponents msg;
 ...
 */
inline nav2_msgs::msg::PathComponents toCoveragePathMsg(
  const Path & raw_path, const F2CField & field, const std_msgs::msg::Header & header,
  const bool is_cartesian)
{
  ...
  for (unsigned int i = 0; i != path.states.size(); i++) {
    ...
    if (
      path.states[i].type != PathSectionType::SWATH &&
...
inline nav_msgs::msg::Path toNavPathMsg(
  const Path & raw_path, const F2CField & field, const std_msgs::msg::Header & header,
  const bool is_cartesian, const float & pt_dist)
{
  using f2c::types::PathSectionType;
  ...
  for (unsigned int i = 0; i != path.states.size(); i++) {
    ...
    if (
      i > 0 && path.states[i].type == PathSectionType::SWATH &&
...
inline nav_msgs::msg::Path toCartesianNavPathMsg(
  const Path & raw_path, const std_msgs::msg::Header & header, const float & pt_dist)
{
 ...
 *
 * Example Use:
    for (nav2_coverage_planner::util::PathComponentsIterator it(msg); it.isValid(); it.advance()) {
...
public:
   ...
   */
  explicit PathComponentsIterator(nav2_msgs::msg::PathComponents & msg)
   ...
   */
  bool isValid() { return idx_ <= max_idx_; }
   ...
   */
  std::pair<nav2_msgs::msg::Swath *, nav_msgs::msg::Path *> getNext()
  {
    if (idx_ < max_idx_ - 1) {
  ...
  nav2_msgs::msg::PathComponents & path_components_;
  unsigned int idx_;
