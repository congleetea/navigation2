...
typedef nav2_msgs::action::ComputeCoveragePath ComputeCoveragePath;

/**
 * @enum Headland computations types
 */
enum class HeadlandType { UNKNOWN = 0, CONSTANT = 1 };

/**
 * @enum Swath computations types
 */
enum class SwathType { UNKNOWN = 0, LENGTH = 1, NUMBER = 2, COVERAGE = 3 };

/**
 * @enum Swath angle types
 */
enum class SwathAngleType { UNKNOWN = 0, SET_ANGLE = 1, BRUTE_FORCE = 2 };

/**
 * @enum Swath computations types
 */
enum class RouteType { UNKNOWN = 0, BOUSTROPHEDON = 1, SNAKE = 2, SPIRAL = 3, CUSTOM = 4 };
 ...
 */
enum class PathContinuityType { UNKNOWN = 0, CONTINUOUS = 1, DISCONTINUOUS = 2 };

class CoverageException : public std::runtime_error
{
public:
  explicit CoverageException(const std::string & description) : std::runtime_error(description) {}
};
