...
#include "nav2_coverage_planner/utils.hpp"
...
public:
  ...
  void activate(NodeT node)
  {
    nav_plan_pub_ = rclcpp::create_publisher<nav_msgs::msg::Path>(
      node->get_node_topics_interface(), "coverage_server/coverage_plan", rclcpp::QoS(1));
    headlands_pub_ = rclcpp::create_publisher<geometry_msgs::msg::PolygonStamped>(
      node->get_node_topics_interface(), "coverage_server/field_boundary", rclcpp::QoS(1));
    planning_field_pub_ = rclcpp::create_publisher<geometry_msgs::msg::PolygonStamped>(
      node->get_node_topics_interface(), "coverage_server/planning_field", rclcpp::QoS(1));
    swaths_pub_ = rclcpp::create_publisher<visualization_msgs::msg::Marker>(
      node->get_node_topics_interface(), "coverage_server/swaths", rclcpp::QoS(1));
  }

  void deactivate();

  void visualize(
    const Field & total_field, const Field & no_headland_field, const Point & ref_pt,
    const nav_msgs::msg::Path & path, const Swaths swaths, const std_msgs::msg::Header & header);
