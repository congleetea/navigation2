...
nav2_util::CallbackReturn CoverageServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  action_server_ = std::make_unique<ActionServer>(
    shared_from_this(), "compute_coverage_path",
    std::bind(&CoverageServer::computeCoveragePath, this), nullptr, std::chrono::milliseconds(500),
    true, server_options);
...
void CoverageServer::computeCoveragePath()
{
  ...
  // 获取抢占式goal.必须满足：如果generate_path，前提必须generate_route.
  ...
  try {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Generating coverage path in %s frame for zone with %zu outer "
      "nodes and %zu inner polygons.",
    ...
    visualizer_->visualize(
      ...
      util::toCartesianNavPathMsg(path, header, path_gen_->getTurnPointDistance()), swaths, header);
...
rcl_interfaces::msg::SetParametersResult CoverageServer::dynamicParametersCallback(
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav2_coverage_planner::CoverageServer)
